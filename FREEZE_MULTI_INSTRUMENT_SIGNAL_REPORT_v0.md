...
- Tests: `407 passed`, one third-party `websockets.legacy` deprecation warning.
...
The deployment audit also hardened the existing operator restart path: a cold start now requires an uncached, active, flat PAPER account proof with no open orders, and the server runs under `caffeinate -is` to prevent sleep-driven heartbeat loss. This changes lifecycle safety only; `scalp_server.py` remains byte-identical.
